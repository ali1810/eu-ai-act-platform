...
async def get_gap_analysis(project: dict, checks: list, score: float, tier: str) -> str:
    """Call LLM for gap analysis — gracefully falls back if no key."""
    ...
    if not key:
        ...
        gaps = "\n".join(f"• {c['article_id']} ({c['article_title']}): {c['findings']}" for c in failed)
        return (
            f"**Compliance Score: {score}% | Risk Tier: {tier.upper()}**\n\n"
            f"The following gaps were identified:\n{gaps}\n\n"
            f"To enable AI-powered analysis, add your GROQ_API_KEY to Streamlit secrets."
        )
    try:
        from openai import AsyncOpenAI
        client = AsyncOpenAI(api_key=key, base_url="https://api.groq.com/openai/v1")
        failed = [c for c in checks if c["status"] in ("non_compliant", "partial")]
        failed_txt = "\n".join(f"- {c['article_id']}: {c['findings']}" for c in failed) or "None"
        resp = await client.chat.completions.create(
            model="llama3-8b-8192",
            temperature=0.2,
            messages=[
                {"role": "system", "content": "You are an EU AI Act compliance expert. Be concise and practical. Max 250 words."},
                {"role": "user",   "content": f"AI project: {project['name']}\nDescription: {project['description']}\nRisk: {tier}\nScore: {score}%\nGaps:\n{failed_txt}\n\nProvide gap analysis and top 3 immediate actions."}
            ]
        )
        return resp.choices[0].message.content or ""
    except Exception as e:
        return f"AI gap analysis unavailable ({e}). Review article findings above for manual assessment."
